   ...
   transition: all 0.3s ease;
	...
	<?php	
		...
		function getSearchResults()
		{
			...
			echo "<div class=\"container-fluid\">";
			echo "<div class=\"row\">";
			foreach(glob("./libs/*") as $yearDir)
			{
				if(substr($yearDir, 7) == "All")
				{
					continue;
				}
				foreach(glob($yearDir."/*") as $schoolYearDir)
				{
					foreach(glob($schoolYearDir."/*") as $file)
					{
						$film = substr($file, 1+strlen($schoolYearDir));
						$schoolYear = substr($schoolYearDir, strlen($yearDir));
						$year = substr($yearDir,7);

						if(fnmatch("*".$_GET['film']."*", $film, FNM_CASEFOLD))
						{
							$noResult = false;
							echo "<div class=\"col-md-4 col-sm-6\">

									<a href=\"./details.php/?year=$year&school_year=$schoolYear&film=$film\"><!--pass the film's directory name to index.php-->
		...
		function createSideBar()
		{
			...
			echo "<form name=\"film\" method=\"get\" action=\"index.php\">
				<input type=\"text\" placeholder=\"Find film by name...\" name=\"film\" size=19>
